...
make_provenance <- function(event, actor = Sys.info()[["user"]], details = list()) {
  tibble::tibble(
    ...
    event = event,
    actor = actor %||% "unknown",
    details = as.character(jsonlite::toJSON(details, auto_unbox = TRUE, null = "null"))
  )
}

append_provenance <- function(x, event, details = list()) {
  existing <- tibble::as_tibble(x$provenance)
  if (!"details" %in% names(existing)) {
    existing$details <- character()
  } else {
    existing$details <- as.character(existing$details)
  }
  new_row <- make_provenance(event, details = details)
  new_row$details <- as.character(new_row$details)
  x$provenance <- dplyr::bind_rows(existing, new_row)
  x
}
...
print.sr_project <- function(x, ...) {
  validate_sr_object(x)
  writeLines(c(
    glue::glue("<sr_project> {x$metadata$name %||% 'unnamed'}"),
    glue::glue("Root: {x$metadata$root %||% 'NA'}")
  ))
  invisible(x)
}
