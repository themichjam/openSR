...
sr_prisma_flow <- function(flow_report) {
  ...
  flow_report <- append_provenance(flow_report, "prisma_flow")
  class(flow_report) <- unique(c("sr_report", "sr_object", class(flow_report)))
  flow_report
}
...
sr_prisma_checklist <- function(type = c("PRISMA 2020", "PRISMA-P", "PRISMA-S")) {
  ...
  class(out) <- unique(c("sr_report", "sr_object", class(out)))
  out
}
...
sr_osf_manifest <- function(project_path = "review") {
  ...
  class(out) <- unique(c("sr_archive", "sr_object", class(out)))
  out
}
