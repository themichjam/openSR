...
sr_screen_template <- function(records, reviewers = c("rev_a", "rev_b"), calibration_round = FALSE) {
  ...
  dat <- tibble::tibble(
    ...
    reviewer_1 = reviewers[[1]],
    decision_1 = NA_character_,
    reviewer_2 = reviewers[[2]],
...
sr_screen_dual <- function(screen_log) {
  ...
  out$data <- dat
  append_provenance(out, "screen_dual", details = list(conflicts = sum(dat$conflict, na.rm = TRUE)))
}
...
sr_screen_resolve <- function(screen_log, resolutions) {
  ...
  screen_log$data <- dat
  append_provenance(screen_log, "screen_resolve", details = list(resolved_n = nrow(res)))
}
