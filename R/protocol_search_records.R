...
sr_protocol_export <- function(protocol, path) {
  ...
  class(protocol) <- unique(c("sr_protocol", "sr_object", class(protocol)))
  protocol
}
...
sr_search_add <- function(search_log,
  ...
  search_log$provenance$details <- as.character(search_log$provenance$details)
  search_log
}
...
sr_deduplicate <- function(records, key_columns = c("title", "doi"), fuzzy = TRUE) {
  ...
  records$provenance$details <- as.character(records$provenance$details)
  records
}
